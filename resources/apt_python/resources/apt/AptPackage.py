...
try:
    # Correct relative import: from resources.apt_python.resources.apt -> up 2 levels -> resources.apt_python
    from ..adapter.adapter import resource_adapter as adapter  # package relative
except Exception:
    _here = Path(__file__).resolve()
    # Path tree: .../DSC/resources/apt_python/resources/apt/AptPackage.py
    ...
    try:
        from resources.apt_python.adapter.adapter import resource_adapter as adapter  # absolute import
    except Exception:
        try:
            # Fallback to direct sibling (if cwd inside resources/apt_python)
            from apt_python.adapter.adapter import resource_adapter as adapter
        except Exception:
            # Final fallback: lightweight shim
...
class AptPackage:
    ...
    def set(self):
        ...
        try:
            ...
            adapter.log("info", f"Set state for package '{self.name}': {set_dict}", "Apt Management", command="set", method="set")
            return set_dict
        except subprocess.CalledProcessError as err:
