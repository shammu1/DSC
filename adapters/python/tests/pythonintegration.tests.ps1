...
function global:Assert-KnownDscAdapterFailure {
    param(
        ...
        $Result.StdErr | Should -Match "resource-type must be provided|Operation Executable 'python3' not found|Failed to run process 'python3'|Command: Resource 'python3' \[exit code 2\]|Resource not found:"
}

function global:Initialize-DscPythonPath {
    ...
    foreach ($pythonCommand in $pythonCommands) {
        try {
            ...
            $python3Path = Join-Path -Path $pythonDir -ChildPath 'python3.exe'
            if (-not (Test-Path -LiteralPath $python3Path)) {
                continue
            }
...
if ($script:DscPython.Resolved) {
    ...
    Write-Host "Unable to resolve a concrete python3.exe for DSC integration tests." -ForegroundColor DarkYellow
}
...
function global:Invoke-DscResourceList {
    param(
...
Describe "Python Adapter - Error Propagation via DSC" {
    It "<CaseName> via <Operation> returns non-zero and stderr" -TestCases @(
        ...
        @{ CaseName = "invalid JSON input"; Operation = "get"; ResourceType = "PythonTest/Get"; InputJson = "{bad-json" }
        @{ CaseName = "invalid JSON input"; Operation = "set"; ResourceType = "PythonTest/Set"; InputJson = "{bad-json" }
        @{ CaseName = "invalid JSON input"; Operation = "test"; ResourceType = "PythonTest/Test"; InputJson = "{bad-json" }
    ) {
