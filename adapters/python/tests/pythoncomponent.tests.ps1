...
# 
# These tests invoke the Python adapter directly via `python -m pyDscAdapter adapter`
# to validate adapter behavior without the DSC engine wrapper.
# 
...
Describe "Python Adapter - Component Tests" {
    BeforeAll {
        ...
        function global:Resolve-ManifestResourcePath {
            param(
                [string]$EntryPath,
                [string]$BaseDir
            )
            ...
            if ([System.IO.Path]::IsPathRooted($EntryPath) -and (Test-Path -LiteralPath $EntryPath)) {
                return (Resolve-Path -LiteralPath $EntryPath).Path
            }

            $asWritten = Join-Path -Path $BaseDir -ChildPath $EntryPath
            if (Test-Path -LiteralPath $asWritten) {
                return (Resolve-Path -LiteralPath $asWritten).Path
            }
            ...
            $probeRoot = $BaseDir
            while ($true) {
                $candidate = Join-Path -Path $probeRoot -ChildPath $tail
                if (Test-Path -LiteralPath $candidate) {
            ...
            throw "Unable to resolve resource path '$EntryPath' from manifest '$BaseDir'."
        }
        ...
        function global:Get-ResourcePathForType {
        ...
        function global:Convert-StdOutToJsonLines {
            param([string]$StdOut)
